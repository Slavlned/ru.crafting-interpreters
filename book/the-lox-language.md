...
Высокоуровневые языки существуют для того, чтобы устранить возможные ошибки,
нудную низкоуровневую работу, а что может быть более утомительным, чем ручное
управление, выделение и освобождение памяти? Никто не хочет просыпаться и 
приветствовать утреннее солнце словами: "Я не могу дождаться момента, когда я пойму
где лучше всего разместить вызов `free()` для каждого байта памяти, что я выделю
сегодня!"

Есть две основные <span name="gc">техники</span> управления памятью:
**подсчет ссылок** и **отслеживающий сборщик мусора** (обычно называемый просто
**"сборщик мусора"** или **"GC"**). Подсчет ссылок гораздо проще в реализации 
-- Я думаю, поэтому Perl, PHP, и Python начали использовать данный метод.
Но, с течением времени, ограничения метода "подсчета ссылок" стали приносить слишком
много хлопот.
Все эти языки в конечном итоге остановились на добавлении полноценного сборщика мусора
или по-крайней мере той его части, которая ответственна за очищение мусора, появляющегося
во время жизни объектов.

<aside name="gc">

На практике, подход с подсчетом ссылок и отслеживания являются более близкими друг к другу
подходами, чем это может показаться. Большинство систем подсчета ссылок используют
отслеживание, чтобы иметь возможность обрабатывать циклы, а барьеры на запись 
поколенческого сборщика мусора выглядят довольно похожими на сохранение вызовов, если
прищуриться.

Для более подробной информации об этом, смотрите "[A Unified Theory of Garbage Collection][gc]" (PDF).

[gc]: http://www.cs.virginia.edu/~cs415/reading/bacon-garbage.pdf

</aside>

У отслеживающего сборщика мусора грозная репутация. Он довольно жестко работает
с памятью напрямую. Отладка GC иногда может провоцировать появление снов
где выкопаетесь в хекс дампах. Но, помните, эта книга для того чтобы рассеять магию
